// attention.rs/src/kernels/src/flash/flash_tq3_store.cu
// TurboQuant 3-bit store kernel for KV cache quantization

#include <cuda_runtime.h>
#include <math.h>
#include <stdint.h>

// TurboQuant 3-bit store kernel
__global__ void flash_tq3_store_kernel(
    const float* __restrict__ K,
    const float* __restrict__ V,
    uint8_t* __restrict__ K_quant,
    uint8_t* __restrict__ V_quant,
    float* __restrict__ K_absmax,
    float* __restrict__ V_absmax,
    const int* __restrict__ slot_mapping,
    int num_tokens,
    int num_kv_heads,
    int head_dim,
    int block_size
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int total_elements = num_tokens * num_kv_heads * head_dim;
    
    if (idx >= total_elements) return;
    
    int token_idx = idx / (num_kv_heads * head_dim);
    int head_idx = (idx / head_dim) % num_kv_heads;
    int dim_idx = idx % head_dim;
    
    float k_val = K[idx];
    float v_val = V[idx];
    
    // Find block and offset within block
    int block_idx = token_idx / block_size;
    int block_offset = token_idx % block_size;
    
    // Quantize K to 3-bit
    uint8_t k_quant = quantize_tq3(k_val);
    K_quant[idx] = k_quant;
    
    // Quantize V to 3-bit
    uint8_t v_quant = quantize_tq3(v_val);
    V_quant[idx] = v_quant;
    
    // Store absmax for dequantization
    int absmax_idx = token_idx * num_kv_heads + head_idx;
    K_absmax[absmax_idx] = fmaxf(K_absmax[absmax_idx], fabsf(k_val));
    V_absmax[absmax_idx] = fmaxf(V_absmax[absmax_idx], fabsf(v_val));
}

// TQ3 quantization function (3-bit symmetric)
__device__ __forceinline__ uint8_t quantize_tq3(float val) {
    // Scale to [-3.5, 3.5] range for 3-bit
    float scaled = val * 4.0f;
    int quant_val = __float2int_rn(scaled);
    quant_val = __max(__min(quant_val, 3), -4);
    // Convert to unsigned 3-bit (0-7)
    return (uint8_t)(quant_val + 4);
}

// TQ3 dequantization function
__device__ __forceinline__ float dequant_tq3(uint8_t quant_val, float absmax) {
    // Convert from unsigned 3-bit (0-7) to signed (-4 to 3)
    int signed_val = (int)quant_val - 4;
    // Scale back to original range
    return (float)signed_val * absmax / 4.0f;
}

// Kernel launcher
extern "C" void launch_flash_tq3_store(
    const float* K,
    const float* V,
    uint8_t* K_quant,
    uint8_t* V_quant,
    float* K_absmax,
    float* V_absmax,
    const int* slot_mapping,
    int num_tokens,
    int num_kv_heads,
    int head_dim,
    int block_size,
    cudaStream_t stream
) {
    int total_elements = num_tokens * num_kv_heads * head_dim;
    int num_blocks = (total_elements + 255) / 256;
    
    flash_tq3_store_kernel<<<num_blocks, 256, 0, stream>>>(
        K, V, K_quant, V_quant, K_absmax, V_absmax,
        slot_mapping, num_tokens, num_kv_heads, head_dim, block_size
    );
}