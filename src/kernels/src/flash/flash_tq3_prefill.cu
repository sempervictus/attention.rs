// attention.rs/src/kernels/src/flash/flash_tq3_prefill.cu
// TurboQuant 3-bit prefill kernel with SM120 optimizations
...
__global__ void flash_tq3_prefill_kernel(
    ...
    // Compute attention with 3-bit quantized K/V
        ...
        for (int m = 0; m < TILE_M; m++) {
            ...
            for (int n = 0; n < TILE_N; n++) {
                ...
                for (int k = 0; k < head_dim; k++) {
                    float k_val = dequant_tq3(s_k_quant[m * head_dim + k], s_k_scale[m]);
                    float v_val = dequant_tq3(s_v_quant[n * head_dim + k], s_v_scale[n]);
...
// Dequantization function for TurboQuant 3-bit
__device__ __forceinline__ float dequant_tq3(uint8_t quant_val, float scale) {
    // Extract 3-bit value (0-7)
    uint8_t val = quant_val & 0x07;
    // Dequantize: val * scale
    return val * scale;
}

// Kernel launcher
extern "C" void launch_flash_tq3_prefill(
    const float* Q,
    ...
    flash_tq3_prefill_kernel<<<grid, block, smem_size, stream>>>(
