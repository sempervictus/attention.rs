...
mod mxfp4_kv_tests {
    ...
    fn make_mxfp4_buffers(dev: &Device) -> (Tensor, Tensor, Tensor, Tensor) {
        let total = NT * NKV;
    ...
    fn mxfp4_store_runs() {
        ...
        let dev = k.device();
        let (k_fp4, k_sf, v_fp4, v_sf) = make_mxfp4_buffers(&dev);
        flash::flash_mxfp4_kv_store(
    ...
    fn mxfp4_sf_nonzero() {
        ...
        assert!(nonzero > sf.len() / 2, "UE8M0 scales must be nonzero for non-zero K data");
        // UE8M0 byte = exp + 127, so for reasonable values it should be in [100, 160]
        let in_range = sf.iter().filter(|&&x| (100..=160).contains(&x)).count();
        assert!(in_range > sf.len() / 2, "most UE8M0 bytes should be in [100,160] for test data");
    }

    #[test]
    fn mxfp4_fp4_codes_valid() {
        let (k, v, slots) = make_data();
        let dev = k.device();
        let (k_fp4, k_sf, v_fp4, v_sf) = make_mxfp4_buffers(&dev);
        flash::flash_mxfp4_kv_store(
