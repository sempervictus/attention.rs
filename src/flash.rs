...
pub fn flash_fp8_rot_store(
    ...
    let dev = match key.device() {
        candle::Device::Cuda(d) => d,
        _ => candle::bail!("flash_fp8_rot_store requires CUDA tensors"),
    };
    let stream = get_cuda_stream(dev);
    ...
    unsafe {
        kernels::ffi::call_flash_fp8_rot_store(
            key_ptr,
            value_ptr,
            key_cache_ptr,
            value_cache_ptr,
...
pub fn flash_fp8_rot_decode(
    ...
    let dev = match query.device() {
        candle::Device::Cuda(d) => d,
        _ => candle::bail!("flash_fp8_rot_decode requires CUDA"),
    };
    let stream = get_cuda_stream(dev);
    ...
    unsafe {
        kernels::ffi::call_flash_fp8_rot_decode(
            q_ptr,
            kc_ptr,
            vc_ptr,
            o_ptr,
...
pub fn flash_nvfp4_kv_prefill(
    ...
    let dev = match query.device() {
        candle::Device::Cuda(d) => d,
        _ => candle::bail!("flash_nvfp4_kv_prefill requires CUDA"),
    };
    let stream = get_cuda_stream(dev);
    ...
    unsafe {
        kernels::ffi::call_flash_nvfp4_kv_prefill(
            q_ptr,
            k_fp4_ptr,
            k_sf_ptr,
            v_fp4_ptr,
